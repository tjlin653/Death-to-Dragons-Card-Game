(function(){
    ...
    let focusIndex = 0;
    const focusable = [instructionsBtn, startgameBtn];

    //tab between home screen buttons
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Tab') {
            e.preventDefault(); 
            //remove focus
            focusable[focusIndex].classList.remove('keyboard-focus');
            ...
            //add focus
            focusable[focusIndex].classList.add('keyboard-focus');
    ...
    document.addEventListener('keydown', function (event) {
        if (!instructionsPage.classList.contains('show')) return;

        if (event.key === 'Escape' || event.key === ' ' || event.key === 'Spacebar' || event.key === 'Enter') {
    ...
    //open audio sources
    const sourcesBtn = document.querySelector('#sources button');
    ...
    document.addEventListener('keydown', function(event) {
        if (!sourcesPage.classList.contains('show')) return;
    ...
    function setUpTurn(){
        ...
        let focusIndexMenu = 0;
        const focusableMenu = [musicBtn, helpBtn, sourcesBtn, quitBtn];

        document.addEventListener('keydown', function(event){
            if (event.key === 'Tab') {
                event.preventDefault(); 
                if (!settingsFocused) {
                    settings.classList.add('keyboard-focus');
                    settings.focus();
                    settingsFocused = true;
                } else {
                    settings.classList.remove('keyboard-focus');
                    settings.blur();
                    settingsFocused = false;
                }
            }
            if (settingsFocused && (event.key === ' ' || event.key === 'Spacebar')) {
                event.preventDefault();
                settings.click();
            }
                if (settingsFocused && (event.key === 'ArrowDown')) {
                    event.preventDefault(); 
                    focusableMenu[focusIndexMenu].classList.remove('keyboard-focus');
                    if (event.shiftKey) {
                        focusIndexMenu = (focusIndexMenu - 1 + focusableMenu.length) % focusableMenu.length;
                    } else {
                        focusIndexMenu = (focusIndexMenu + 1) % focusableMenu.length;
                    }
                    focusableMenu[focusIndexMenu].classList.add('keyboard-focus');
                    focusableMenu[focusIndexMenu].focus();
                }
                if (settingsFocused && (event.key === 'ArrowUp')) {
                    event.preventDefault(); 
                    focusableMenu[focusIndexMenu].classList.remove('keyboard-focus');
                    focusIndexMenu = (focusIndexMenu - 1 + focusableMenu.length) % focusableMenu.length;
                    focusableMenu[focusIndexMenu].classList.add('keyboard-focus');
    ...
    function displayHand(){
        ...
        displayCards.innerHTML += `<img id="effectCard" src="images/${gameData.effectFile}"> <img id="gemCard" src="images/${gameData.gemFile}">`;

        document.querySelector('#gemCard').addEventListener('click', chooseGemCard);
        document.querySelector('#effectCard').addEventListener('click', chooseEffectCard);

        cardSound.play();

        //arrow key hover effects
        document.addEventListener('keydown', function(event){
            if(event.key === 'ArrowLeft'){
                event.preventDefault();
                if(!effectCard.classList.contains('keyboard-focus')){
                    effectCard.classList.add('keyboard-focus');
                    gemCard.classList.remove('keyboard-focus');
                }
            }
            if(event.key === 'ArrowRight'){
                event.preventDefault();
                if(!gemCard.classList.contains('keyboard-focus')){
                    gemCard.classList.add('keyboard-focus');
                    effectCard.classList.remove('keyboard-focus');
                }
            }
            //press spacebar to click the hovered card
        ...
        effectCard.addEventListener('mouseenter', function() {
            gemCard.classList.remove('keyboard-focus');
        });
        gemCard.addEventListener('mouseenter', function() {
            effectCard.classList.remove('keyboard-focus');
        });
        document.addEventListener('click', function(e){
            if(e.target !== effectCard && e.target !== gemCard){
                effectCard.classList.remove('keyboard-focus');
                gemCard.classList.remove('keyboard-focus');
            }
        });
    ...
    function addEndScreenKeyboardControls(endScreen) {
        const endButton = endScreen.querySelector('.endingBtn');
        ...
        function keyHandler(event) {
            if (event.key === 'Tab') {
                event.preventDefault();
                endButton.classList.add('keyboard-focus');
                endButton.focus();
            }
            if (event.key === ' ' || event.key === 'Spacebar') {
                event.preventDefault();
    ...
    function playerWins() {
        ...
        addEndScreenKeyboardControls(winScreen);
    }
    ...
    function playerLoses() {
        ...
        screenBG.style.backgroundSize = "cover";
        screenBG.style.backgroundPosition = "center top";
        addEndScreenKeyboardControls(loseScreen);
    }
})();